...
> - 🪟 **Windows (推荐/原生)**：现在已经全面支持原生 Windows 环境，请**打开 PowerShell**（而非 CMD）直接执行 `.\start-chrome-debug.ps1` 等脚本；如果在 CMD 中，请使用 `powershell -File .\start-chrome-debug.ps1`，无需强制安装 WSL2。（如果仍然希望使用 WSL2：安装 `wsl --install`，后续按 Linux 流程操作）。
...
| macOS/Linux 脚本        | Windows 脚本 (PowerShell)| 用途             | 使用场景                                                     |
| ----------------------- | ------------------------ | -------------- | -------------------------------------------------------- |
| `start-chrome-debug.sh` | `start-chrome-debug.ps1` | 启动 Chrome 调试 | 步骤 2：打开浏览器，端口 9222，供各平台登录与 onboard 连接                    |
| `onboard.sh`            | `onboard.ps1`            | 配置认证向导     | 步骤 4、5：选择平台（deepseek-web 等），捕获 Cookie/Token              |
| `server.sh`             | `server.ps1`             | 管理 Gateway 服务| 步骤 6 及日常：`start` / `stop` / `restart` / `status`，端口 3001 |
...
看到认证成功就可以了，如果要增加其他模型，再运行一次 onboard 脚本即可。
```
